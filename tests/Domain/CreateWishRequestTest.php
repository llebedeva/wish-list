<?php
namespace App\Tests\Domain;

use App\Domain\CreateWishRequest;
use PHPUnit\Framework\TestCase;

class CreateWishRequestTest extends TestCase
{
    private const VALID_WISH = 'I wish something';
    private const VALID_LINK = 'https://ru.wikipedia.org/wiki/URL';
    private const VALID_DESCRIPTION = 'text message about my wish';
    private const VALID_PRIORITY = 1;

    private const TOO_LONG_WISH = 'RIXviHKDcR9RZry5x8EyrM117hnBGWFdoTTNPEiVhe3qDODHjJwX75sFdRJ502KtkdLJ65PKcIjjHN5O8MU2IwlMgCtQAdP4xMxA9';
    private const TOO_LONG_LINK = '';
    private const TOO_LONG_DESCRIPTION = '';

    public function testSuccessIfVaidAgruments()
    {
        $wish = self::VALID_WISH;
        $link = self::VALID_LINK;
        $description = self::VALID_DESCRIPTION;
        $priority = self::VALID_PRIORITY;

        $request = new CreateWishRequest($wish, $link, $description, $priority);

        $this->assertInstanceOf(CreateWishRequest::class, $request);
    }

    /**
     * @dataProvider providerInvalidArguments
     * @param $wish
     * @param $link
     * @param $description
     * @param $priority
     */
    public function testThrowsExceptionIfInvalidArguments($wish, $link, $description, $priority)
    {
        $this->expectException(\InvalidArgumentException::class);

        new CreateWishRequest($wish, $link, $description, $priority);
    }

    public function providerInvalidArguments()
    {
        return [
            ['', self::VALID_LINK, self::VALID_DESCRIPTION, self::VALID_PRIORITY],
            [self::VALID_WISH, self::VALID_LINK, self::VALID_DESCRIPTION, ''],

            [self::TOO_LONG_WISH, self::VALID_LINK, self::VALID_DESCRIPTION, self::VALID_PRIORITY],
            [self::VALID_WISH, self::TOO_LONG_LINK, self::VALID_DESCRIPTION, self::VALID_PRIORITY],
            [self::VALID_WISH, self::VALID_LINK, self::TOO_LONG_DESCRIPTION, self::VALID_PRIORITY],

            [null, self::VALID_LINK, self::VALID_DESCRIPTION, self::VALID_PRIORITY],
            [self::VALID_WISH, null, self::VALID_DESCRIPTION, self::VALID_PRIORITY],
            [self::VALID_WISH, self::VALID_LINK, null, self::VALID_PRIORITY],
            [self::VALID_WISH, self::VALID_LINK, self::VALID_DESCRIPTION, null]
        ];
    }
}
